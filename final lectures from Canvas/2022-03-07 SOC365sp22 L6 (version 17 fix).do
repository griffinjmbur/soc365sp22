	...
	*II.ii. Bivariate analysis (two quant. vars.)
		*II.ii.a. Graphical analysis
			...
			* linear regression soon.
			...
			bidensity stress goodwork, levels(10) 
			/*levels(#) dictates number of
...
*III. Multivariate analysis. 
	...
	*III.ii. Informal numeric methods for multivariate analysis. 
	...
	table GWbands female bedpartrmmt if ~missing(bedpartrmmt), stat(mean stress)
	...
	* Note! This does not work in Stata 17. I only discovered that this morning;
	* they seem to have left a bit of a hole in the code here, so I'll look into
	* this for you all. An easy workaround would be to just set the Stata version
	* to 15, but it looks like they did not put together a quick fix. 
	
	* That's OK because my point was that this is a bit messy anyways. 
	
	* table GWbands female bedpartrmmt if ~missing(bedpartrmmt) ///
		* & ~missing(RA4Q10A1), by(RA4Q10A1) stat(mean stress)
	...
	* We could, e.g., run two totally separate regression models
	...
	table female bedpartrmmt, stat(mean stress)
